...
const UpdateDestination = () => {
  ...
  useEffect(()=>{
    axios
    .get(
      `http://localhost:1001/countries/country/cities/city/destinations/${params.destinationID}`,
    )
    .then((response) => {
        setDestination(response.data);
        setDestinationName(response.data.name);
        setPrice(response.data.price);
        setType(response.data.type_of_accommodation);
  ...
  const saveDestination = (obj: any) => {
    axios
      .put(
        `http://localhost:1001/countries/country/city/destination/${params.destinationID}/update`,
        {
    ...
    <Container sx={{ marginTop: "50px", marginBottom: "50px" }}>
      <Card sx={{ display: "flex" }}>
        <Box sx={{ display: "flex", flexDirection: "column" }}>
          <TransitionAlerts label={'Destination updated successfully.'} isOpen={isOpen} setOpen={(type: any)=> setNotifOpen(type)}></TransitionAlerts>
